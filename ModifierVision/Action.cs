        ...
        public static void OnDraw(EventArgs args)
        {
            ...
            foreach (var heroModifier in Members.System)
            {
                ...
                foreach (var modifier in modList.Where(x=>x!=null && x.IsValid))
                {
                    ...
                    /*if (remTime<=1)
                        continue;*/
                    var itemPos = startPos + new Vector2(2, -2 + size.X*counter);
                    Drawing.DrawRect(itemPos, size,
                        Textures.GetTexture($"materials/ensage_ui/modifier_textures/round/{modifier.TextureName}.vmat"));
                    //Drawing.DrawRect(itemPos, size,
                        ///Color.Black,true);
                    ...
                    var textPos = itemPos + new Vector2(size.Y, size.Y - textSize.Y);
